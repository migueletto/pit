...
typedef struct {
  ...
  int (*text)(image_t *image, int x, int y, char *font, double size, double angle, char *text,
              int *bounds);

  int (*save)(image_t *image, char *filename);

  int (*save_exif)(image_t *image, char *filename, int mode, time_t ts, double lon, double lat,
                   double alt);

  int (*show)(image_t *image, int fullscreen, window_provider_t *wp);

  int (*event)(image_t *image, int *key, int *mods, int *buttons);
