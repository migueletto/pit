...
PIT_LIB_FUNCTION(builtin,simulate)
  PIT_LIB_PARAM_S(service)
  PIT_LIB_PARAM_I(id)
  PIT_LIB_PARAM_S(host)
  PIT_LIB_PARAM_I(port)
PIT_LIB_CODE
  bt_provider_t *bt;
  bt = script_get_pointer(PIT_LIB_PE, BT_PROVIDER);
  r = sim_add(service, id, host, port, bt);
  PIT_LIB_FREE_S(service);
  PIT_LIB_FREE_S(host);
PIT_LIB_END_B
