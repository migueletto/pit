...
void *sys_lib_load(char *libname, int *first_load) {
  ...
  *first_load = 0;
...
#ifdef WINDOWS
  char buf[FILE_PATH];
  normalize_path(libname, buf, FILE_PATH);
  ...
  // check if library is already loaded
  dlerror();
  lib = dlopen(libname, RTLD_NOW | RTLD_NODELETE | RTLD_NOLOAD);

  if (lib == NULL) {
    // not loaded yet: load it
    dlerror();
    lib = dlopen(libname, RTLD_NOW | RTLD_NODELETE);

    if (lib != NULL) {
      debug(DEBUG_INFO, "SYS", "library %s loaded", libname);
      *first_load = 1;
    } else {
    ...
    // already loaded
    debug(DEBUG_INFO, "SYS", "library %s already loaded", libname);
  }
#endif
